(function() {
    // 代码块1：判断是在浏览器环境还是nodeJS环境，方便代码3后期使用
    ...
    var util = {
        ...
        addCSS: function(cssText) {
            ...
            style.head = 'text/css'; //这里必须显示设置style元素的type属性为text/css，否则在ie中不起作用
            if(style.styleSheet) { //IE
                var func = function() {
                    try {
                        style.styleSheet.cssText = cssText;
                    }catch(e) { //防止IE中stylesheet数量超过限制而发生错误
    ...
    //代码块2： 声明了一个对象SliderTool,将用户传进来的option和默认的defaultOption进行合并
    function SliderTool(options) {
        this.options = util.extend({}, this.constructor.defaultOptions, options);
        this.init();
        this.bindEvents();
        this.diffX = 0;
        this.flag = false; //是否滑动到最右侧
    }
    SliderTool.defaultOptions = {
        el: document.body //默认放到body里
    }
    var proto = SliderTool.prototype = new EventEmitter(); //SliderTool继承emitter
    proto.constructor = SliderTool; //修正构造器
    proto.init = function() {
    ...
    if(typeof exports != 'undefined' && !exports.nodeType) {
        if (typeof module != 'undefined' && !module.nodeType && module.exports) {
            exports = module.exports = SliderTool;
        }
        exports.SliderTools = SliderTool;
    }else{
        root.SliderTool = SliderTool;
    } 
 }());
